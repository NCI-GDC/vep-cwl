...
from postgres.time import Time, TimeWga
...
def run_build_slurm_scripts(args):
    ...
    # Load template
    template_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'etc/template.wga.sh')
    template_str  = None
    ...
    try:
        cases = postgres.status.get_vep_inputs_from_status(engine, 'vep_wga_cwl_status', 'wga_vep_input')
        write_slurm_script(cases, args, template_str)
    except NoSuchTableError:
        cases = postgres.status.get_all_vep_inputs(engine, 'wga_vep_input')
        write_slurm_script(cases, args, template_str)
    ...
    for case in cases:
        ...
        if all([dat.src_vcf_id, dat.src_vcf_location, dat.case_id,
           ...
           dat.workflow_id_02, dat.workflow_name_02, dat.workflow_description_02, dat.workflow_version_02]):
            ...
            slurm = os.path.join(args.outdir, 'vep_wga_cwl.{0}.sh'.format(dat.src_vcf_id))
...
def run_cwl(args):
    ...
    uniqdir = tempfile.mkdtemp(prefix="vep_wga_%s_" % str(vcf_uuid), dir=args.basedir)
    ...
    #setup logger
    log_file = os.path.join(workdir, "%s.vep.wga.cwl.log" %str(vcf_uuid))
    ...
    # Run CWL
        ...
        # Set metrics table
        met = TimeWga(case_id = args.case_id,
...
def get_args():
    ...
    p_slurm.add_argument('--s3dir', default="s3://vep_4", help='s3bin for output files [s3://vep_4/]')
