...
from postgres.time import Time, TimeMutectDbsnp
...
def run_build_slurm_scripts(args):
    ...
    # Load template
    template_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'etc/template.mutect2_dbsnp_rerun.sh')
    template_str  = None
    ...
    try:
        cases = postgres.status.get_vep_inputs_from_status(engine, 'vep_mutect2_dbsnp_rerun_cwl_status', 
            'mutect2_dbsnp_rerun_vep_input')
        write_slurm_script(cases, args, template_str)
    except NoSuchTableError:
        cases = postgres.status.get_all_vep_inputs(engine, 'mutect2_dbsnp_rerun_vep_input')
        write_slurm_script(cases, args, template_str)
    ...
    for case in cases:
        ...
        if all([dat.src_vcf_id, dat.src_vcf_location, dat.case_id,
           ...
           dat.workflow_id_02, dat.workflow_name_02, dat.workflow_description_02, dat.workflow_version_02]):
            ...
            slurm = os.path.join(args.outdir, 'vep_mutect2_dbsnp_rerun_cwl.{0}.sh'.format(dat.src_vcf_id))
...
def run_cwl(args):
    ...
    uniqdir = tempfile.mkdtemp(prefix="vep_mutect2_dbsnp_rerun_%s_" % str(vcf_uuid), dir=args.basedir)
    ...
    #setup logger
    log_file = os.path.join(workdir, "%s.vep.mutect2_dbsnp_rerun.cwl.log" %str(vcf_uuid))
    ...
    # Tables
    table     = postgres.status.VEPMutectDbsnpStatus
    timeTable = TimeMutectDbsnp
    ...
    if s3_exit_code != 0:
        ...
        postgres.status.set_download_error(s3_exit_code, args.case_id, str(vcf_uuid), 
            ...
            logger, hostname, table=table, timeTable=timeTable)
    ...
    # Run CWL
        ...
        # Set metrics table
        met = timeTable(case_id = args.case_id,
        ...
        postgres.status.add_status(engine, args.case_id, str(vcf_uuid), args.src_vcf_id,  
                                  ...
                                  loc, datetime_now, md5, hostname, table=table)
...
def get_args():
    ...
    p_slurm.add_argument('--s3dir', default="s3://vep_4/mutect2dbsnp", help='s3bin for output files [s3://vep_4/]')
    ...
    p_run.add_argument('--s3dir', default="s3://vep_4", help='s3bin for uploading output files')
